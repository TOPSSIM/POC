    ...
    const char *scheme_output)
{
    ...
    if (ogs_plmn_id_mcc(&ogs_local_conf()->serving_plmn_id) &&
        ogs_plmn_id_mnc(&ogs_local_conf()->serving_plmn_id)) {
    ...
    if (test_ue->requested_nssai.num_of_s_nssai == 0) {
        ogs_nas_s_nssai_ie_t *s_nssai = &test_ue->requested_nssai.s_nssai[0];
        s_nssai->sst = 1;
        s_nssai->sd.v = OGS_S_NSSAI_NO_SD_VALUE;
        s_nssai->mapped_hplmn_sst_presence = false;
        s_nssai->mapped_hplmn_sst = 0;
        s_nssai->mapped_hplmn_sd.v = OGS_S_NSSAI_NO_SD_VALUE;
        test_ue->requested_nssai.num_of_s_nssai = 1;
    }
