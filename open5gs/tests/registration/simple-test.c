...
static void log_ue_ipv4(test_sess_t *sess)
{
    char *ip = NULL;
    ...
    if (!sess->ue_ip.ipv4) {
        abts_log_message("[simple-test] UE IPv4: (not assigned)");
        return;
    }

    ip = ogs_ipv4_to_string(sess->ue_ip.addr);
    if (ip) {
        abts_log_message("[simple-test] UE IPv4: %s", ip);
        ogs_free(ip);
    } else {
        abts_log_message("[simple-test] UE IPv4: (conversion failed)");
    }
}

static void log_plmn_ids(
    test_ue_t *test_ue,
    ogs_nas_5gs_mobile_identity_suci_t *mobile_identity_suci)
{
    ogs_plmn_id_t hplmn_id;
    char hplmn[OGS_PLMNIDSTRLEN];
    char vplmn[OGS_PLMNIDSTRLEN];

    ogs_assert(test_ue);
    ogs_assert(mobile_identity_suci);

    memset(&hplmn_id, 0, sizeof(hplmn_id));
    ogs_nas_to_plmn_id(&hplmn_id, &mobile_identity_suci->nas_plmn_id);

    abts_log_message("[simple-test] HPLMN ID: %s (from SUCI)",
            ogs_plmn_id_to_string(&hplmn_id, hplmn));
    abts_log_message("[simple-test] VPLMN ID: %s (from UE TAI)",
            ogs_plmn_id_to_string(&test_ue->nr_tai.plmn_id, vplmn));
}
...
static void test1_func(abts_case *tc, void *data)
{
    ...
    log_plmn_ids(test_ue, &mobile_identity_suci);
