...
void udm_ue_state_operational(ogs_fsm_t *s, udm_event_t *e)
{
    ...
    case OGS_EVENT_SBI_SERVER:
        ...
        CASE(OGS_SBI_SERVICE_NAME_NUDM_SDM)
            SWITCH(message->h.method)
            CASE(OGS_SBI_HTTP_METHOD_GET)
                ogs_info("[DBG][UDM][Nudm_SDM_Get][REQ] SUPI[%s] resource[%s] "
                        "method[%s]",
                        udm_ue->supi ? udm_ue->supi : "(null)",
                        message->h.resource.component[1] ?
                            message->h.resource.component[1] : "(all-datasets)",
                        message->h.method ? message->h.method : "(null)");
                ...
                CASE(OGS_SBI_RESOURCE_NAME_SM_DATA)
                    if (udm_topssim_sdm_cache_try_send(
                                udm_ue, stream, message) == true)
                        break;
