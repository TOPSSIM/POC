...
void ogs_sbi_xact_remove(ogs_sbi_xact_t *xact)
{
    ...
    if (xact->trace_method)
        ogs_free(xact->trace_method);
    if (xact->trace_uri)
        ogs_free(xact->trace_uri);
    if (xact->trace_service_name)
        ogs_free(xact->trace_service_name);
    if (xact->trace_resource)
        ogs_free(xact->trace_resource);
...
static void sbi_xact_trace_clear(ogs_sbi_xact_t *xact)
{
    ogs_assert(xact);

    if (xact->trace_method) {
        ogs_free(xact->trace_method);
        xact->trace_method = NULL;
    }
    if (xact->trace_uri) {
        ogs_free(xact->trace_uri);
        xact->trace_uri = NULL;
    ...
    if (xact->trace_service_name) {
        ogs_free(xact->trace_service_name);
        xact->trace_service_name = NULL;
    }
    if (xact->trace_resource) {
        ogs_free(xact->trace_resource);
        xact->trace_resource = NULL;
    }
}

static char *sbi_request_resource_string(ogs_sbi_request_t *request)
{
    char *resource = NULL;
    int i;
    ...
    for (i = 0; i < OGS_SBI_MAX_NUM_OF_RESOURCE_COMPONENT &&
        ...
        if (!resource) {
            resource = ogs_strdup(request->h.resource.component[i]);
            ogs_assert(resource);
        } else {
            next = ogs_msprintf("%s/%s",
                    resource, request->h.resource.component[i]);
            ogs_assert(next);
            ogs_free(resource);
            resource = next;
        }
...
void ogs_sbi_xact_trace_begin(
        ogs_sbi_xact_t *xact, ogs_sbi_request_t *request)
{
    ogs_assert(xact);
    ogs_assert(request);

    sbi_xact_trace_clear(xact);

    xact->trace_started_at = ogs_time_now();
    xact->trace_sequence++;

    xact->trace_method = ogs_strdup(request->h.method ?
            request->h.method : "-");
    ogs_assert(xact->trace_method);

    xact->trace_uri = ogs_strdup(request->h.uri ? request->h.uri : "-");
    ogs_assert(xact->trace_uri);

    xact->trace_service_name = ogs_strdup(request->h.service.name ?
            request->h.service.name : "-");
    ogs_assert(xact->trace_service_name);

    xact->trace_resource = sbi_request_resource_string(request);
    ogs_assert(xact->trace_resource);

    ogs_info("[TOPSSIM][SBI-TIMING][REQ] XID[%d:%u] NF[%s] "
            "service[%s] method[%s] resource[%s] uri[%s]",
            xact->id,
            xact->trace_sequence,
            OpenAPI_nf_type_ToString(xact->requester_nf_type),
            xact->trace_service_name,
            xact->trace_method,
            xact->trace_resource,
            xact->trace_uri);
}

void ogs_sbi_xact_trace_response(
        ogs_sbi_xact_t *xact, int status, ogs_sbi_response_t *response)
{
    double elapsed_ms = 0;
    int http_status = 0;
    ...
    if (!xact->trace_started_at)
        return;

    elapsed_ms = (double)(ogs_time_now() - xact->trace_started_at) / 1000.0;
    if (response)
        http_status = response->status;

    ogs_info("[TOPSSIM][SBI-TIMING][RSP] XID[%d:%u] NF[%s] "
            "service[%s] method[%s] resource[%s] status[%d] "
            "http_status[%d] elapsed_ms[%.3f] uri[%s]",
            xact->id,
            xact->trace_sequence,
            OpenAPI_nf_type_ToString(xact->requester_nf_type),
            xact->trace_service_name ? xact->trace_service_name : "-",
            xact->trace_method ? xact->trace_method : "-",
            xact->trace_resource ? xact->trace_resource : "-",
            status,
            http_status,
            elapsed_ms,
            xact->trace_uri ? xact->trace_uri : "-");
}
