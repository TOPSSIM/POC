        ...
        int status, ogs_sbi_response_t *response, void *data)
{
    ...
    if (xact_id >= OGS_MIN_POOL_ID && xact_id <= OGS_MAX_POOL_ID) {
        ...
        if (xact)
            ogs_sbi_xact_trace_response(xact, status, response);
    }
 ...
 */
    ...
    if (scp_client) {
        ...
        if (client) {
            ...
            ogs_expect(rc == true);
            if (rc == true)
                ogs_sbi_xact_trace_begin(xact, request);
        ...
        ogs_sbi_client_t *client, ogs_sbi_xact_t *xact)
{
    ...
    if (client->fqdn && ogs_sbi_fqdn_in_vplmn(client->fqdn) == true) {
        ...
        if (!sepp_client && !nrf_client) {
            ...
            if (rc == true)
                ogs_sbi_xact_trace_begin(xact, nrf_request);
        ...
        ogs_sbi_request_t *request, void *data)
{
    ...
    if (rc == true && client_cb == ogs_sbi_client_handler) {
        ogs_pool_id_t xact_id = OGS_POINTER_TO_UINT(data);
        if (xact_id >= OGS_MIN_POOL_ID && xact_id <= OGS_MAX_POOL_ID) {
