    ...
    ogs_sbi_message_t *recvmsg)
{
    ...
    CASE(OGS_SBI_RESOURCE_NAME_AM_DATA)
        ...
        ogs_assert(response);
        ogs_assert(true == ogs_sbi_server_send_response(stream, response));
        ogs_info("[DBG][UDM][Nudm_SDM_Get][RSP] SUPI[%s] resource[%s] "
                "status[%d]",
                udm_ue->supi ? udm_ue->supi : "(null)",
                OGS_SBI_RESOURCE_NAME_AM_DATA, recvmsg->res_status);
    ...
    CASE(OGS_SBI_RESOURCE_NAME_SMF_SELECTION_SUBSCRIPTION_DATA)
        ...
        ogs_info("[DBG][UDM][Nudm_SDM_Get][RSP] SUPI[%s] resource[%s] "
                "status[%d]",
                udm_ue->supi ? udm_ue->supi : "(null)",
                OGS_SBI_RESOURCE_NAME_SMF_SELECT_DATA, recvmsg->res_status);
