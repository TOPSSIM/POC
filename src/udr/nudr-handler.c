    ...
    if (processSmfSel) {
        ...
        for (i = 0; i < subscription_data.num_of_slice; i++) {
            ...
            for (j = 0; j < slice_data->num_of_session; j++) {
                ...
                /*
                 * Always include lboRoamingAllowed explicitly.
                 * This avoids ambiguity on the AMF side when the value is false.
                 */
                DnnInfo->is_lbo_roaming_allowed = true;
                DnnInfo->lbo_roaming_allowed = session->lbo_roaming_allowed;
