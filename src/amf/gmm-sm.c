        ...
        CASE(OGS_SBI_SERVICE_NAME_NAMF_COMM)
            ...
            CASE(OGS_SBI_RESOURCE_NAME_UE_CONTEXTS)
                ...
                CASE(OGS_SBI_RESOURCE_NAME_TRANSFER)
                    ...
                    if (sbi_message->res_status == OGS_SBI_HTTP_STATUS_OK) {
                        ogs_info("[DBG][N14][RX][UEContextTransfer-RSP] UE[%s] HTTP[%d]",
                                AMF_UE_HAVE_SUPI(amf_ue) ? amf_ue->supi :
                                (AMF_UE_HAVE_SUCI(amf_ue) ? amf_ue->suci : "Unknown"),
                    ...
                    if (amf_ue->amf_ue_context_transfer_state ==
                            UE_CONTEXT_TRANSFER_NEW_AMF_STATE) {
                        ogs_info("[%s] Keep transferred SM context(s) on new AMF",
                                amf_ue->supi);
                        r = amf_ue_sbi_discover_and_send(
                                OGS_SBI_SERVICE_TYPE_NAUSF_AUTH, NULL,
    ...
    case AMF_EVENT_5GMM_MESSAGE:
        ...
        case OGS_NAS_5GS_REGISTRATION_REQUEST:
            ...
            if (gmm_registration_request_from_old_amf(amf_ue,
                ...
                ogs_info("[DBG][N14][TX][UEContextTransfer] UE[%s] old-guti[PLMN:0x%x AMF_ID:0x%x M_TMSI:0x%x]",
                        AMF_UE_HAVE_SUPI(amf_ue) ? amf_ue->supi :
                        (AMF_UE_HAVE_SUCI(amf_ue) ? amf_ue->suci : "Unknown"),
                        ogs_plmn_id_hexdump(&guami.plmn_id),
                        ogs_amf_id_hexdump(&guami.amf_id),
        ...
        case OGS_NAS_5GS_SERVICE_REQUEST:
            ...
            if (!(AMF_UE_HAVE_SUCI(amf_ue) || AMF_UE_HAVE_SUPI(amf_ue))) {
                ogs_info("Service request : Unknown UE (no SUCI/SUPI)");
                r = nas_5gs_send_service_reject(ran_ue, amf_ue,
                    OGS_5GMM_CAUSE_UE_IDENTITY_CANNOT_BE_DERIVED_BY_THE_NETWORK
                    );
                ogs_expect(r == OGS_OK);
         ...
         */
            ...
            if (amf_ue->amf_ue_context_transfer_state ==
                ...
                */
                ...
                ogs_info("[DBG][N14][TX][RegistrationStatusUpdate] UE[%s] status[TRANSFERRED] old-guti[PLMN:0x%x AMF_ID:0x%x M_TMSI:0x%x]",
                        AMF_UE_HAVE_SUPI(amf_ue) ? amf_ue->supi :
                        (AMF_UE_HAVE_SUCI(amf_ue) ? amf_ue->suci : "Unknown"),
                        ogs_plmn_id_hexdump(&guami.plmn_id),
                        ogs_amf_id_hexdump(&guami.amf_id),
            ...
            CASE(OGS_SBI_RESOURCE_NAME_REGISTRATIONS)
                ...
                CASE(OGS_SBI_HTTP_METHOD_PUT)
                    if (amf_topssim_sdm_cache_try_handle(
                            amf_ue, state,
                            OGS_SBI_RESOURCE_NAME_AM_DATA) == true)
                        break;
 ...
 */
            ...
            ogs_info("[DBG][Nudm_SDM_Get][RSP] SUPI[%s] status[%d] method[%s] "
                    "resource[%s]",
                    amf_ue->supi ? amf_ue->supi : "(null)",
                    sbi_message->res_status,
                    sbi_message->h.method ? sbi_message->h.method : "(null)",
                    sbi_message->h.resource.component[1] ?
    ...
    case OGS_FSM_ENTRY_SIG:
        ...
        if (amf_ue->amf_ue_context_transfer_state ==
            ...
            */
            ...
            ogs_info("[DBG][N14][TX][RegistrationStatusUpdate] UE[%s] status[NOT_TRANSFERRED] old-guti[PLMN:0x%x AMF_ID:0x%x M_TMSI:0x%x]",
                    AMF_UE_HAVE_SUPI(amf_ue) ? amf_ue->supi :
                    (AMF_UE_HAVE_SUCI(amf_ue) ? amf_ue->suci : "Unknown"),
                    ogs_plmn_id_hexdump(&guami.plmn_id),
                    ogs_amf_id_hexdump(&guami.amf_id),
