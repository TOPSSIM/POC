...
static void amf_log_nudm_sdm_json(
        const char *supi, const char *resource, cJSON *item)
{
    char *content = NULL;
    ...
    if (content) {
        ogs_info("[DBG][Nudm_SDM_Get][BODY] SUPI[%s] resource[%s] %s",
                supi ? supi : "(null)", resource ? resource : "(null)",
                content);
        ogs_free(content);
    } else {
        ogs_warn("[DBG][Nudm_SDM_Get][BODY] SUPI[%s] resource[%s] "
        ...
        amf_ue_t *amf_ue, int state, ogs_sbi_message_t *recvmsg)
{
    ...
    CASE(OGS_SBI_RESOURCE_NAME_AM_DATA)
        ...
        if (recvmsg->AccessAndMobilitySubscriptionData) {
            cJSON *item = OpenAPI_access_and_mobility_subscription_data_convertToJSON(
                    recvmsg->AccessAndMobilitySubscriptionData);
            if (item) {
                amf_log_nudm_sdm_json(amf_ue->supi,
                        OGS_SBI_RESOURCE_NAME_AM_DATA, item);
            } else {
                ogs_warn("[DBG][Nudm_SDM_Get][BODY] SUPI[%s] resource[%s] "
                        "convertToJSON() failed",
        ...
        if (amf_topssim_sdm_cache_try_handle(
                amf_ue, state,
                OGS_SBI_RESOURCE_NAME_SMF_SELECT_DATA) == false) {
    ...
    CASE(OGS_SBI_RESOURCE_NAME_SMF_SELECT_DATA)
        if (recvmsg->SmfSelectionSubscriptionData) {
            cJSON *item = OpenAPI_smf_selection_subscription_data_convertToJSON(
                    recvmsg->SmfSelectionSubscriptionData);
            if (item) {
                amf_log_nudm_sdm_json(amf_ue->supi,
                        OGS_SBI_RESOURCE_NAME_SMF_SELECT_DATA, item);
            } else {
                ogs_warn("[DBG][Nudm_SDM_Get][BODY] SUPI[%s] resource[%s] "
                        "convertToJSON() failed",
                        amf_ue->supi ? amf_ue->supi : "(null)",
                            ...
                            if (DnnInfoList) {
                                OpenAPI_list_for_each(DnnInfoList, node2) {
                                    ...
                                    if (DnnInfo) {
                                        ...
                                        /*
                                         * Default to HR-safe value unless UDM
                                         * explicitly provides lboRoamingAllowed.
                                         */
                                        session->lbo_roaming_allowed = false;
                                        slice->num_of_session++;
        ...
        if (amf_topssim_sdm_cache_try_handle(
                amf_ue, state,
                OGS_SBI_RESOURCE_NAME_UE_CONTEXT_IN_SMF_DATA) == false) {
            r = amf_ue_sbi_discover_and_send(
                    OGS_SBI_SERVICE_TYPE_NUDM_SDM, NULL,
                    amf_nudm_sdm_build_get,
                    amf_ue, state,
    ...
    CASE(OGS_SBI_RESOURCE_NAME_UE_CONTEXT_IN_SMF_DATA)
        if (recvmsg->UeContextInSmfData) {
            cJSON *item = OpenAPI_ue_context_in_smf_data_convertToJSON(
                    recvmsg->UeContextInSmfData);
            if (item) {
                amf_log_nudm_sdm_json(amf_ue->supi,
                        OGS_SBI_RESOURCE_NAME_UE_CONTEXT_IN_SMF_DATA, item);
            } else {
                ogs_warn("[DBG][Nudm_SDM_Get][BODY] SUPI[%s] resource[%s] "
                        "convertToJSON() failed",
                        amf_ue->supi ? amf_ue->supi : "(null)",
        ...
        if (amf_topssim_sdm_cache_enabled()) {
            ogs_info("[TOPSSIM][AMF-SDM-CF] Skip runtime SDM subscription "
                    "SUPI[%s]", amf_ue->supi);
            r = amf_ue_sbi_discover_and_send(
