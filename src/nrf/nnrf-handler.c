...
static bool discover_uri_has_param(const char *uri, const char *name);
static char *discover_cache_key_from_uri(const char *uri);
static bool send_discover_cache_hit(
        ogs_sbi_stream_t *stream,
        ogs_sbi_message_t *recvmsg,
        ogs_sbi_discovery_option_t *discovery_option,
        OpenAPI_search_result_t *SearchResult,
        nrf_disc_cache_entry_t *cache_entry,
        ogs_sbi_message_t *sendmsg);

#define NRF_PRE_DISCOVERY_PARAM_NAME "pre-discovery"
#define NRF_PRE_DISCOVERY_CACHE_MAX_AGE 1800
        ...
        ogs_sbi_stream_t *stream, ogs_sbi_message_t *recvmsg)
{
    ...
    OpenAPI_nf_profile_t *NFProfile = NULL;
    OpenAPI_lnode_t *node = NULL;
    nrf_disc_cache_entry_t *cache_entry = NULL;
    char *cache_key = NULL;
    bool pre_discovery = false;
    int i;
    ...
    pre_discovery = discover_uri_has_param(
            recvmsg->h.uri, NRF_PRE_DISCOVERY_PARAM_NAME);
    cache_key = discover_cache_key_from_uri(recvmsg->h.uri);
    ...
    if (SearchResult->nf_instances->count) {
        ...
        if (cache_key) {
            cache_entry = nrf_disc_cache_find(cache_key);
            if (cache_entry) {
                ogs_info("NF-Discover cache hit [%s]", cache_key);
                if (send_discover_cache_hit(stream, recvmsg, discovery_option,
                        SearchResult, cache_entry, &sendmsg) == true) {
                    goto cleanup;
                }
                ogs_warn("NF-Discover cache stale, fallback to H-NRF [%s]",
                        cache_key);
            }
        }
        ...
        assoc->disc_cache_key = cache_key;
        cache_key = NULL;
        assoc->disc_cache_max_age = pre_discovery ?
            NRF_PRE_DISCOVERY_CACHE_MAX_AGE : 0;
        ...
        int status, ogs_sbi_response_t *response, void *data)
{
    ...
    if (message.SearchResult->is_validity_period &&
            message.SearchResult->validity_period > 0)
        max_age = message.SearchResult->validity_period;
    if (assoc->disc_cache_key)
        nrf_disc_cache_update(
                assoc->disc_cache_key, message.SearchResult,
                assoc->disc_cache_max_age > 0 ?
                    assoc->disc_cache_max_age : max_age);
...
static bool discover_query_name_equals(
        const char *param, size_t param_len, const char *name)
{
    const char *eq = NULL;
    size_t name_len = 0;
    size_t key_len = 0;
    ...
    eq = memchr(param, '=', param_len);
    key_len = eq ? (size_t)(eq - param) : param_len;

    return key_len == name_len && !strncmp(param, name, name_len);
}
...
static bool discover_uri_has_param(const char *uri, const char *name)
{
    ...
    while (*p) {
        ...
        if (discover_query_name_equals(p, len, name))
            return true;
...
static char *discover_cache_key_from_uri(const char *uri)
{
    ...
    base_len = (size_t)(q - uri);
    memcpy(key, uri, base_len);
    w = key + base_len;
    ...
    while (*p) {
        ...
        if (!discover_query_name_equals(
                    p, len, NRF_PRE_DISCOVERY_PARAM_NAME)) {
            *w++ = first ? '?' : '&';
            memcpy(w, p, len);
            w += len;
            first = false;
...
{
    ogs_sbi_response_t *response = NULL;
    ogs_time_t remaining;
    bool service_map = false;
    int max_age;
    int i, n;
    ...
    ogs_assert(recvmsg);
    ogs_assert(SearchResult);
    ogs_assert(cache_entry);
    ogs_assert(sendmsg);

    remaining = cache_entry->expires_at - ogs_get_monotonic_time();
    if (remaining <= 0)
        return false;

    max_age = (int)ogs_time_to_sec(remaining);
    if (max_age <= 0)
        max_age = 1;

    service_map = discovery_option &&
        OGS_SBI_FEATURES_IS_SET(discovery_option->requester_features,
                OGS_SBI_NNRF_DISC_SERVICE_MAP);

    n = 0;
    for (i = 0; i < cache_entry->num_nf_instance_id; i++) {
        ogs_sbi_nf_instance_t *nf_instance = NULL;
        OpenAPI_nf_profile_t *NFProfile = NULL;

        if (recvmsg->param.limit && n >= recvmsg->param.limit)
            break;

        if (!cache_entry->nf_instance_id[i])
            continue;

        nf_instance = ogs_sbi_nf_instance_find(cache_entry->nf_instance_id[i]);
        if (!nf_instance)
            continue;
        ...
        NFProfile = ogs_nnrf_nfm_build_nf_profile(
                nf_instance, NULL, discovery_option, service_map);
        if (!NFProfile)
            continue;
    ...
    if (recvmsg->param.limit)
        SearchResult->num_nf_inst_complete = n;

    if (cache_entry->num_nf_instance_id > 0 &&
            SearchResult->nf_instances->count == 0)
        return false;

    if (SearchResult->nf_instances->count) {
        SearchResult->is_validity_period = true;
        SearchResult->validity_period = max_age;

        sendmsg->SearchResult = SearchResult;
        sendmsg->http.cache_control = ogs_msprintf("max-age=%d", max_age);
        ogs_assert(sendmsg->http.cache_control);
    } else {
        sendmsg->SearchResult = SearchResult;
    }
